...
fn file_paths() {
    ...
    let mut url = Url::from_file_path(&path::posix::Path::new("/foo/bar"))
                 .unwrap_or_else(|_| panic!("failed to create URL"));
    ...
    let mut url = Url::from_file_path(&path::windows::Path::new(r"C:\foo\bar"))
                 .unwrap_or_else(|_| panic!("failed to create URL"));
    assert_eq!(url.host(), Some(&Host::Domain("".to_string())));
...
fn directory_paths() {
    ...
    let url = Url::from_directory_path(&path::posix::Path::new("/foo/bar"))
             .unwrap_or_else(|_| panic!("failed to create URL"));
    assert_eq!(url.host(), Some(&Host::Domain("".to_string())));
    assert_eq!(url.path(), Some(["foo".to_string(), "bar".to_string(), "".to_string()].as_slice()));

    let url = Url::from_directory_path(&path::windows::Path::new(r"C:\foo\bar"))
             .unwrap_or_else(|_| panic!("failed to create URL"));
    assert_eq!(url.host(), Some(&Host::Domain("".to_string())));
